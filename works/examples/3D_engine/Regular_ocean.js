...
export class Ocean extends THREE.Mesh {
    constructor(params = {}) {
        ...
        let waterGeometry = new THREE.PlaneBufferGeometry(
            size,
            size,
            segments,
            segments
		...
		*/
        ...
        this.add(
            typeof this.water == undefined
                ? new THREE.AxisHelper(600)
                : this.water
        );
        ...
        if (params.parentGUI) {
            ...
            let pcos = new Proxy(
                /*ptarget*/ {},
                {
                    ...
                    set: function (obj, prop, value) {
                        ...
                        scope.wavCre.setWaveDef(
                            (2 * Math.PI) / scope.currentCos["T"],
                            scope.currentCos["A"],
                            scope.currentCos["theta"]
                        );
            ...
            this.conf
                .add(pcos, "theta", 0, 360, 0.01)
                .name("<div>Direction (&theta;)</div>");
            this.conf
                .add(pcos, "phi", -180, 180, 0.01)
                .name("<div>Phase (&Phi;)</div>");
